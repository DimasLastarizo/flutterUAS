-- ═══════════════════════════════════════════════════════════════════════════
-- Leaderboard: level = level game (user_level_progress), bukan level XP profil
-- ═══════════════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public.game_level_for_user(p_user_id uuid)
RETURNS int
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COALESCE(
    ...
    (
      ...
      FROM user_level_progress
      WHERE user_id = p_user_id AND status = 'selesai'
    ),
    1
  );
$$;

GRANT EXECUTE ON FUNCTION public.game_level_for_user(uuid) TO anon, authenticated;

-- Baca progress user lain untuk tampilan level di leaderboard (SELECT saja).
CREATE POLICY "progress_read_leaderboard" ON user_level_progress
  FOR SELECT TO anon, authenticated
  USING (true);

-- Backfill entri leaderboard yang masih menyimpan level XP.
UPDATE leaderboard_entries
SET level = public.game_level_for_user(user_id)
WHERE user_id IS NOT NULL;

-- ─── submit_level_result: simpan level game ke leaderboard ─────────────────
